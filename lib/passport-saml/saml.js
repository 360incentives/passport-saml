...
SAML.prototype.getIssuerFromAssertion = function (container) {

  var xml = new Buffer(container.SAMLResponse, 'base64').toString('utf8');
  var doc = new xmldom.DOMParser().parseFromString(xml);

  var issuer = xpath(doc, "/*[local-name()='Response']/*[local-name()='Assertion']/*[local-name()='Issuer']/text()");

  if (issuer && issuer.length) {
    return issuer[0].nodeValue;
  }
  return false;
};
